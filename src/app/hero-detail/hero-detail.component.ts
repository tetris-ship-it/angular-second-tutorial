...
export class HeroDetailComponent {
   ...
   getHero():void{
    ...
    this.heroService.getHero(id).subscribe(hero=>this.hero=hero);//the difference between Observable and Promise is that an observable is lazy meaning that when and how to use the data can be controlled but promise activates the moment the code runs. Plus for multiple value returns using an observable is advisable but when single values are returned using a promise is better.
   ...
   goBack():void{
    this.location.back();//this method is useful for going back by using the goBack() method
   }
}
